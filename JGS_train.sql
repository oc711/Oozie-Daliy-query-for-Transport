...
drop table if exists z_lab_drm_hive_temp.dm_train_jgs;
create table if not exists  z_lab_drm_hive_temp.dm_train_jgs as
with mobile as 
( 
	select * from  z_lab_drm_hive_temp.dm_ae_daliy_route 
	where (d_tm is not null and distance is not null and vitesse is not null) and d_tm !=0 and distance_diag >=10 and 
...
select
	...
	a.nom_axe,
	a.troncon,
	ST_DISTANCE(geo_wkt , lon_lat_point) as dist_pt_to_geo_train
from   
  (
  ...
  CROSS JOIN 
  (
   select ST_GeomFromText(wkt) as geo_wkt,type,nom_axe,troncon
   from z_lab_drm_hive_socle.ref_reseau_ferre_nettoye
   where type in ('TER','TGV_CLASSIQUE','TGV_LGV')
   ) a;
...
DROP TABLE if exists z_lab_drm_hive_temp.dm_ae_train_bi_session; 
CREATE TABLE if not exists  z_lab_drm_hive_temp.dm_ae_train_bi_session as 
with a as
	(
		SELECT *,
		CASE WHEN (gps_tm - LAG(gps_tm,1,gps_tm) over (PARTITION BY sim_imsi ORDER BY row_num) <= 3600 ) THEN 0 ELSE -3 END as bi_score_1,
		CASE WHEN (
					(vitesse >= 30 and vitesse <= 400)  and 
					(
						(dist_pt_to_geo_train<= 0.001 and 
						LAG(dist_pt_to_geo_train) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001 and
						LAG(dist_pt_to_geo_train,2) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001) 
						or
						(dist_pt_to_geo_train <= 0.001 and 
						LEAD(dist_pt_to_geo_train) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001 and
						LEAD(dist_pt_to_geo_train,2) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001)
						or
						(dist_pt_to_geo_train <= 0.001 and 
						LEAD(dist_pt_to_geo_train) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001 and
						LAG(dist_pt_to_geo_train)  over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001 )
					)
				  )
	    THEN 1 ELSE 0 END AS bi_score_2,
		CASE WHEN(
					(
						(
							(vitesse >= 30 or vitesse <= 400) and
							dist_pt_to_geo_train > 0.001 and 
							LAG(dist_pt_to_geo_train) over (PARTITION BY sim_imsi ORDER BY row_num) > 0.001 and
							LAG(dist_pt_to_geo_train,2) over (PARTITION BY sim_imsi ORDER BY row_num) > 0.001
						) 
	    ...
	    THEN 0 ELSE 1 END AS bi_score_3,
	    CASE WHEN (
						(vitesse >= 30 and vitesse <= 400)
						or
						(
							(
								dist_pt_to_geo_train <= 0.001 and 
								(
									( LAG(dist_pt_to_geo_train,2) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001 or LEAD(dist_pt_to_geo_train,2) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001)
									or 
									( LAG(dist_pt_to_geo_train) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001 or LEAD(dist_pt_to_geo_train) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001 )
								)
							) 
							or
							(
								dist_pt_to_geo_train > 0.001 and 
								(
									( LAG(dist_pt_to_geo_train,2) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001 and LEAD(dist_pt_to_geo_train,2) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001)
									or 
									( LAG(dist_pt_to_geo_train) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001 and LEAD(dist_pt_to_geo_train) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001 )
									or
									( LAG(dist_pt_to_geo_train) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001 and LEAD(dist_pt_to_geo_train,2) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001)
									or
									( LAG(dist_pt_to_geo_train,2) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001 and LEAD(dist_pt_to_geo_train) over (PARTITION BY sim_imsi ORDER BY row_num) <= 0.001)
								)
							) 
						)
		...
		FROM  z_lab_drm_hive_temp.dm_ae_train_resultat
		-- WHERE month = ${M}  and day = ${D}
),
b as  
(	    SELECT 
		a.*, 
...
select 
	   sim_imsi,gps_tm,dist_pt_to_geo_train,vitesse,bi_score_session_large,
...
DROP TABLE IF EXISTS  z_lab_drm_hive_temp.dm_ae_train_session; 
CREATE TABLE if not exists  z_lab_drm_hive_temp.dm_ae_train_session as 
with l as 
(
	SELECT 
	...
	FROM  z_lab_drm_hive_temp.dm_ae_train_bi_session
),
r as 
(
	SELECT 
...
--
INSERT INTO z_lab_drm_hive_temp.dimeng_ae_train_session
